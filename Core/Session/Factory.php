...
class Factory
{
     ...
     */
    public static function getInstance($strSessionLayer = 'db')
    {
        $strClassName = __NAMESPACE__.'\\'.ucfirst(strtolower($strSessionLayer)).'\\Session';

        if (!class_exists($strClassName)) throw new Core\Exception("No class '{$strClassName}' found for {$strSessionLayer} layer");
        ...
        $objSession = $objReflection->newInstanceArgs();
